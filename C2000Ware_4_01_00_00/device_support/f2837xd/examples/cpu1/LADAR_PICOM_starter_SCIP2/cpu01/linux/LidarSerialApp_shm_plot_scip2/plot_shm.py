...
lidar_dist_pong = np.zeros((228))
for i in range(228):
    lidar_dist_pong[i] = ((3*i+44)*0.3515625-135)*math.pi/180 #calculate the angle based on datasheet
...
def animate(i):
    try:
        if (fcn.my_sem_trywait() == 0):
            shm_file = open('/dev/shm/posix-shared-mem-lidar-dist','rb') #open the shared memory
...
def init():
    ...
    line.set_xdata(lidar_dist_pong) # xdata is the angle
    return line,
