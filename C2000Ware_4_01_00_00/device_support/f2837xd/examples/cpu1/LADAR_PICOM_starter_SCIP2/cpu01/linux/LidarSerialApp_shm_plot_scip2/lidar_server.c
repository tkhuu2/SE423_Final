...
#define SHARED_MEM_NAME "/posix-shared-mem-lidar-dist"

struct shared_memory
{
  float lidar_dist[228];
};
...
int run_app()
{
  ...
  int count = 0;
  char mychar;
  float lidar_dist_pong[228];
  float lidar_dist_ping[228];

  int to_xy = 0; //convert distance to xy
  int read_lidar_state = 0;         //states for lidar reading, initialized as wait for S command
  ...
  for (ii = 0; ii < 228; ii++)
  {
    shared_mem_ptr->lidar_dist[ii] = 0;
  }
        ...
        { //get actual data
          ...
          else
          {
            ...
            else if (get_dis_first == 0)
            {
              ...
              if (pingpong == 0)
              {
                lidar_dist_ping[dis_index % 228] = distance;
              }
              else if (pingpong == 1)
              {
                lidar_dist_pong[dis_index % 228] = distance;
              }
              dis_index += 1;
              get_dis_first = 1;
            }
          ...
          { //two consecutive line feed means the end of the current data stream
            ...
            if (dis_index > 5)
            {
              for (ii = 0; ii < 228; ii++)
              {
                if (pingpong == 1)
                {
                  shared_mem_ptr->lidar_dist[ii] = lidar_dist_ping[ii];
                }
                else
                {
                  shared_mem_ptr->lidar_dist[ii] = lidar_dist_pong[ii];
                }
              }
