...
char G_command[] = "GS0044072503\n"; //command for getting distance -120 to 120 degree
uint16_t G_len = 13; //length of command
...
float LADARtemp_x = 0;
float LADARtemp_y = 0;
uint16_t status_error = 0;
char status_error_temp[30];
...
__interrupt void RXCINT_recv_ready(void)
{
    ...
    if (RXCdata & 0xC000) {
        ...
        if (read_ladar_state == 0) {//wait for the starting char 'G'
            ...
            if (G_count == 14) { //status
                if (RXCdata != '0') {
                    read_ladar_state = 99; //error state
                    status_error_temp[status_error%30] = RXCdata;
                    status_error += 1;
                }
            }
            if (G_count == 15) {
            ...
            if (G_count >= 23) {//there are total 23 things before the data
                read_ladar_state = 2;
                get_dis_first = 1;
                dis_index = 0;
...
//                    }
        ...
        } else if (read_ladar_state == 99) {
            status_error_temp[status_error%30] = RXCdata;
            status_error += 1;
            status_error_temp[status_error%30] = 0x0a;
            status_error += 1;
            read_ladar_state = 0;
